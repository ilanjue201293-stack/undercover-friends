...
function chooseWords(settings: Settings) {
  if (settings.mode === "mrwhite") {
    ...
    return { civilianWord: pair[Math.floor(Math.random() * 2)], infiltratorWord: null };
  }
  ...
  if (settings.wordSource === "custom") {
    ...
    return {
      civilianWord: flip ? a : b,
      infiltratorWord: flip ? b : a,
    };
  }
  ...
  const flip = Math.random() < 0.5;
  return {
    civilianWord: flip ? pair[0] : pair[1],
    infiltratorWord: flip ? pair[1] : pair[0],
  };
}
...
export function startGame(room: Room) {
  ...
  if (room.settings.mode === "impostor" && room.settings.wordSource === "custom") {
    if (!room.settings.customWordA.trim() || !room.settings.customWordB.trim()) {
      throw new Error("Entre les deux mots personnalisés.");
    }
  ...
  for (const p of active) {
    if (infiltratorIds.has(p.id)) {
      p.role = "infiltrator";
      p.word = room.settings.mode === "mrwhite" ? null : words.infiltratorWord;
    } else {
      p.role = "civil";
      p.word = words.civilianWord;
    }
  }
  ...
  room.game = {
    ...
    votes: {},
    clues: [],
    visibleClueStartIndex: 0,
    usedClueKeys: [],
    tieCandidates: [],
    tiebreakIteration: 0,
    pendingMrWhiteId: null,
    civilianWord: words.civilianWord,
    infiltratorWord: words.infiltratorWord,
    infiltratorCountInitial: infiltratorCount,
    lastVoteReveal: null,
    winner: null,
    winReason: null,
  };
...
function beginClueRound(room: Room, round: number, preserveCycle: boolean) {
  ...
  room.game.clueRound = round;
  room.game.order = newOrder(room);
  room.game.turnIndex = 0;
  room.game.turnStartedAt = now();
  room.game.voteStartedAt = null;
  room.game.votes = {};
  room.game.tieCandidates = [];
  room.game.pendingMrWhiteId = null;
  if (!preserveCycle && round === 1) {
    // cycle already incremented by caller when needed
  }
...
function beginNextCycle(room: Room) {
  if (!room.game) return;
  room.game.cycle += 1;
  room.game.clueRound = 1;
  room.game.votes = {};
  room.game.tieCandidates = [];
  room.game.tiebreakIteration = 0;
  room.game.pendingMrWhiteId = null;
  if (!room.settings.keepCluesAfterVote) {
    room.game.visibleClueStartIndex = room.game.clues.length;
  }
  beginClueRound(room, 1, false);
}
...
function finishClueSequence(room: Room) {
  const game = room.game;
  if (!game) return;
  if (game.phase === "tiebreak-clue") {
    beginTiebreakVote(room);
    return;
  }
  if (game.clueRound < room.settings.roundsBeforeVote) {
    beginClueRound(room, game.clueRound + 1, true);
  } else {
    beginVote(room);
  }
}
...
export function submitClue(room: Room, player: Player, raw: string) {
  ...
  game.clues.push({
    ...
    kind: game.phase === "tiebreak-clue" ? "tiebreak" : "normal",
    at: now(),
  });
  game.turnIndex += 1;
  game.turnStartedAt = now();
  if (game.turnIndex >= game.order.length) finishClueSequence(room);
  else skipUnavailableTurns(room);
}
...
function eligibleVoters(room: Room) {
  ...
  if (game.phase !== "tiebreak-vote") return alive;
...
export function submitVote(room: Room, voter: Player, targetId: string) {
  ...
  maybeResolveVotesEarly(room);
}

function connectedEligibleVoters(room: Room) {
  return eligibleVoters(room).filter((p) => p.connected);
}

function maybeResolveVotesEarly(room: Room) {
  const game = room.game;
  if (!game || (game.phase !== "vote" && game.phase !== "tiebreak-vote")) return;
  const voters = connectedEligibleVoters(room);
  if (voters.length > 0 && voters.every((p) => Boolean(game.votes[p.id]))) resolveVotes(room);
}
...
function resolveVotes(room: Room) {
  ...
  const targetIds = new Set(
    game.phase === "tiebreak-vote" ? game.tieCandidates : alivePlayers(room).map((p) => p.id)
  );
  const entries = Object.entries(game.votes).filter(([voterId, targetId]) => validVoterIds.has(voterId) && targetIds.has(targetId));
  const tally: Record<string, number> = {};
  for (const [, targetId] of entries) tally[targetId] = (tally[targetId] ?? 0) + 1;

  if (entries.length === 0) {
    game.lastVoteReveal = { at: now(), votes: [], tally: {}, eliminatedId: null, tiedIds: [] };
    addEvent(room, "Aucun vote reçu : personne n'est éliminé.", "vote");
    beginNextCycle(room);
    return;
  }
  ...
  if (top.length === 1) {
    reveal.eliminatedId = top[0];
    eliminate(room, top[0]);
    return;
  }

  game.tieCandidates = top;
  game.tiebreakIteration += 1;
  const names = top.map((id) => room.players.find((p) => p.id === id)?.name ?? "?").join(", ");
  addEvent(room, `Égalité entre ${names}. Ils doivent donner un indice supplémentaire.`, "vote");
  beginTiebreakClues(room);
}
...
function beginTiebreakClues(room: Room) {
  ...
  game.order = shuffle(game.tieCandidates.filter((id) => {
    const p = room.players.find((x) => x.id === id);
    return p && !p.isEliminated && !p.isSpectator;
  }));
  game.turnIndex = 0;
  game.turnStartedAt = now();
  game.voteStartedAt = null;
  game.votes = {};
  skipUnavailableTurns(room);
}
...
function beginTiebreakVote(room: Room) {
  const game = room.game;
  if (!game) return;
  game.phase = "tiebreak-vote";
  game.votes = {};
  game.voteStartedAt = now();
  game.turnStartedAt = null;
  addEvent(room, "Revote de départage ouvert.", "vote");
  maybeResolveVotesEarly(room);
}

function countRoles(room: Room) {
  const alive = alivePlayers(room);
  const infiltrators = alive.filter((p) => p.role === "infiltrator").length;
  const civilians = alive.filter((p) => p.role === "civil").length;
  return { infiltrators, civilians };
}

function checkParityWin(room: Room) {
  const counts = countRoles(room);
  if (counts.infiltrators > 0 && counts.infiltrators >= counts.civilians) {
    endGame(room, "infiltrators", `${roleLabel(room)}${counts.infiltrators > 1 ? "s" : ""} aussi nombreux que les civils.`);
    return true;
  }
  return false;
}
...
function eliminate(room: Room, playerId: string) {
  ...
  const isInfiltrator = player.role === "infiltrator";
  const revealed = isInfiltrator ? roleLabel(room) : "Civil";
  addEvent(room, `${player.name} est éliminé : ${revealed}.`, "vote");

  if (isInfiltrator) {
    const remaining = countRoles(room).infiltrators;
    addEvent(room, `Il reste ${remaining} ${roleLabel(room)}${remaining > 1 ? "s" : ""} en jeu.`, "game");
    if (room.settings.mode === "mrwhite") {
      game.phase = "mrwhite-guess";
      game.pendingMrWhiteId = player.id;
      game.turnStartedAt = null;
      game.voteStartedAt = null;
      addEvent(room, `${player.name} a une tentative pour deviner le mot des civils.`, "game");
      return;
    }
    if (remaining === 0) {
      endGame(room, "civilians", "Tous les imposteurs ont été éliminés.");
      return;
    }
    beginNextCycle(room);
    return;
...
export function submitMrWhiteGuess(room: Room, player: Player, guess: string) {
  ...
  if (correct) {
    addEvent(room, `${player.name} a trouvé le mot « ${game.civilianWord} » !`, "win");
    endGame(room, "infiltrators", "Mr White a deviné le mot des civils après son élimination.");
    return;
  }

  addEvent(room, `${player.name} n'a pas trouvé le mot des civils.`, "game");
  game.pendingMrWhiteId = null;
  const remaining = countRoles(room).infiltrators;
  if (remaining === 0) {
    endGame(room, "civilians", "Tous les Mr White ont été éliminés et aucun n'a trouvé le mot.");
    return;
  }
  beginNextCycle(room);
}
...
export function updatePresenceAndTimers(room: Room, currentPlayerId?: string) {
  ...
  if (current) {
    const wasAway = !current.connected;
    current.connected = true;
    current.lastSeen = t;
    current.leaveSignaledAt = null;
    if (wasAway) addEvent(room, `${current.name} est revenu.`, "join");
  }
  ...
  for (const p of room.players) {
    ...
    if (signaledGone || heartbeatGone) {
      p.connected = false;
      addEvent(room, `${p.name} n'est plus sur le site.`, "leave");
    }
  }
  ...
  if (!room.players.find((p) => p.id === room.hostId)?.connected) {
    ...
    if (next && next.id !== room.hostId) {
      room.hostId = next.id;
      addEvent(room, `${next.name} devient l'hôte de la room.`, "info");
    }
  }
  ...
  if (game.phase === "clue" || game.phase === "tiebreak-clue") {
    skipUnavailableTurns(room);
    if (!room.game || (room.game.phase !== "clue" && room.game.phase !== "tiebreak-clue")) return;
    const started = room.game.turnStartedAt ?? t;
    if (t - started >= room.settings.actionTimeSec * 1000) {
      const id = currentTurnId(room.game);
      const p = room.players.find((x) => x.id === id);
      if (p) addEvent(room, `Temps écoulé pour ${p.name} : tour passé.`, "game");
      room.game.turnIndex += 1;
      room.game.turnStartedAt = t;
      if (room.game.turnIndex >= room.game.order.length) finishClueSequence(room);
      else skipUnavailableTurns(room);
    }
    return;
  }
  ...
  if (game.phase === "vote" || game.phase === "tiebreak-vote") {
    maybeResolveVotesEarly(room);
    if (!room.game || (room.game.phase !== "vote" && room.game.phase !== "tiebreak-vote")) return;
    const started = room.game.voteStartedAt ?? t;
    if (t - started >= room.settings.actionTimeSec * 1000) resolveVotes(room);
  }
}
...
export function publicState(room: Room, requester: Player) {
  ...
  const canKnowTeammates = requester.role === "infiltrator"
    && room.settings.infiltratorsKnowEachOther
    && (room.settings.mode === "mrwhite" || room.settings.impostorKnowsRole);
  ...
  const phaseDeadline = game
    ? (game.phase === "clue" || game.phase === "tiebreak-clue") && game.turnStartedAt
      ? game.turnStartedAt + room.settings.actionTimeSec * 1000
      : (game.phase === "vote" || game.phase === "tiebreak-vote") && game.voteStartedAt
        ? game.voteStartedAt + room.settings.actionTimeSec * 1000
        : null
    : null;
  ...
  return {
    ...
    players: room.players.map((p) => ({
      ...
      revealedRole: p.isEliminated || room.status === "gameover" ? p.role : null,
    })),
    me: {
      id: requester.id,
      name: requester.name,
      isHost: requester.id === room.hostId,
      ready: requester.ready,
      connected: requester.connected,
      isSpectator: requester.isSpectator,
      isEliminated: requester.isEliminated,
      role: selfRole,
      word: room.status === "playing" || room.status === "gameover" ? requester.word : null,
      revealAck: requester.revealAck,
      knownTeammateIds: canKnowTeammates
        ? room.players.filter((p) => p.id !== requester.id && p.role === "infiltrator").map((p) => p.id)
        : [],
    },
    game: game ? {
      phase: game.phase,
      ...
      order: game.order,
      submittedVoterIds: Object.keys(game.votes),
      eligibleVoterIds: voters.map((p) => p.id),
      allowedTargetIds: targets.map((p) => p.id),
      clues: visibleClues,
      tieCandidates: game.tieCandidates,
      tiebreakIteration: game.tiebreakIteration,
      pendingMrWhiteId: game.pendingMrWhiteId,
      phaseDeadline,
      lastVoteReveal: game.lastVoteReveal,
      winner: game.winner,
