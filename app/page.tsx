...
function countInfiltrators(n: number) {
...
function playerName(state: PublicState, id: string | null) {
...
export default function Home() {
  ...
  if (!session) {
    return <Landing nickname={nickname} setNickname={setNickname} joinCode={joinCode} setJoinCode={setJoinCode} onCreate={createRoom} onJoin={joinRoom} busy={busy} error={error} />;
  }

  if (!state) {
    return <main className="shell centerPage"><div className="loader" /><p className="muted">Connexion à la room {session.code}…</p>{error && <div className="errorBox">{error}</div>}</main>;
  }

  const remaining = state.game?.phaseDeadline
    ? Math.max(0, Math.ceil((state.game.phaseDeadline - (clock + serverOffset.current)) / 1000))
    : null;

  return (
    <main className="shell">
      <RoomHeader state={state} onLeave={leaveRoom} onCopy={() => {
        navigator.clipboard?.writeText(state.code).then(() => setToast("Code copié")).catch(() => setToast(`Code : ${state.code}`));
      }} />
      {error && <div className="errorBox">{error}</div>}
      {toast && <div className="toast">{toast}</div>}

      {state.status === "lobby" ? (
        <Lobby state={state} busy={busy} perform={perform} />
      ) : (
        <Game state={state} busy={busy} perform={perform} remaining={remaining} />
      )}
    </main>
  );
}
...
function Landing(props: {
  ...
  onCreate: () => void; onJoin: () => void; busy: boolean; error: string;
}) {
  const submitJoin = (e: FormEvent) => { e.preventDefault(); props.onJoin(); };
  return (
    <main className="landing">
      <section className="hero">
        <div className="eyebrow"><span className="dot" /> PARTIE PRIVÉE ENTRE POTES</div>
      ...
      <div className="miniRules">
        <span>3+ joueurs</span><i>•</i><span>aucun compte</span><i>•</i><span>Imposteur & Mr White</span>
      </div>
    </main>
  );
}
...
function RoomHeader({ state, onLeave, onCopy }: { state: PublicState; onLeave: () => void; onCopy: () => void }) {
  return (
    <header className="roomHeader">
      ...
      <button className="roomCode" onClick={onCopy} title="Copier le code"><small>ROOM</small><strong>{state.code}</strong><span>⧉</span></button>
    ...
    <div className="lobbyGrid">
      <section className="mainColumn">
        <div className="sectionHead">
          <div><span className="kicker">LOBBY</span><h2>{connectedActive.length} joueur{connectedActive.length > 1 ? "s" : ""}</h2></div>
          <div className="countPill">{infiltrators} {state.settings.mode === "mrwhite" ? "Mr White" : "imposteur"}{infiltrators > 1 ? "s" : ""}</div>
        </div>

        <div className="playerGrid">
          {[...state.players].sort((a, b) => a.joinOrder - b.joinOrder).map((p) => (
            <div className={`playerCard ${!p.connected ? "offline" : ""}`} key={p.id}>
        ...
        <div className="readyBar">
          <button className={state.me.ready ? "secondaryButton readyButton" : "primaryButton"} disabled={busy || state.me.isSpectator} onClick={() => perform("ready", { ready: !state.me.ready })}>
            {state.me.ready ? "✓ Prêt" : "Je suis prêt"}
          </button>
          {state.me.isHost ? (
            <button className="launchButton" disabled={busy || !allReady} onClick={() => perform("start")}>{allReady ? "Lancer la partie →" : connectedActive.length < 3 ? "3 joueurs minimum" : "En attente des joueurs"}</button>
          ) : (
            <div className="waitingText">{allReady ? "Tout le monde est prêt — l'hôte peut lancer." : "Mets-toi prêt quand tu as fini de regarder les règles."}</div>
          )}
        </div>
      </section>
  ...
  return (
    <section className="settingsPanel">
      ...
      <div className="segmented">
        ...
        <button className={s.mode === "mrwhite" ? "active" : ""} onClick={() => patch({ mode: "mrwhite" })} disabled={busy}>Mr White</button>
      </div>
      ...
      {s.mode === "impostor" && <>
        ...
        {s.wordSource === "custom" ? (
          <div className="customWords">
            ...
            <p className="settingHelp">Le jeu choisira au hasard lequel devient civil. Ce mode force l'imposteur à ne pas savoir qu'il l'est.</p>
          </div>
        ) : (
          <Toggle label="L'imposteur sait qu'il l'est" checked={s.impostorKnowsRole} onChange={(v) => patch({ impostorKnowsRole: v })} disabled={busy} />
        )}
      </>}
      ...
      <div className="settingSplit">
        ...
        <div><label className="settingLabel">Temps par action</label><select className="selectInput" value={s.actionTimeSec} onChange={(e) => patch({ actionTimeSec: Number(e.target.value) })} disabled={busy}>{[15,20,25,30,35,40,45,50,55,60].map(v => <option value={v} key={v}>{v}s</option>)}</select></div>
      </div>
...
function ReadOnlySettings({ state }: { state: PublicState }) {
  const s = state.settings;
  return (
    <section className="settingsPanel readonly">
      ...
      <RuleRow a="Timer" b={`${s.actionTimeSec} secondes`} />
...
function Game({ state, busy, perform, remaining }: { state: PublicState; busy: boolean; perform: (a: string, x?: Record<string, unknown>) => Promise<void>; remaining: number | null }) {
  ...
  return (
    <div className="gameGrid">
      ...
      <aside className="sideColumn gameSide">
        <PlayersInGame state={state} />
        <Activity state={state} />
      </aside>
    </div>
  );
...
function PhaseHeader({ state, remaining }: { state: PublicState; remaining: number | null }) {
  ...
  return (
    <div className="phaseHeader">
      ...
      {remaining !== null && <div className={`timer ${remaining <= 5 ? "urgent" : ""}`}><span>{remaining}</span><small>sec</small></div>}
    </div>
  );
}
...
function Reveal({ state, busy, perform }: { state: PublicState; busy: boolean; perform: (a: string, x?: Record<string, unknown>) => Promise<void> }) {
  ...
  const visibleRole = roleName(state, state.me.role);
  const isMrWhite = state.settings.mode === "mrwhite" && state.me.role === "infiltrator";
  const teammates = state.me.knownTeammateIds.map((id) => playerName(state, id));

  return (
    <div className="secretWrap">
      {!revealed ? (
        <button className="secretCover" onClick={() => setRevealed(true)}><span>◉</span><strong>Révéler mon rôle</strong><small>Assure-toi que personne ne regarde ton écran.</small></button>
      ) : (
        <div className={`secretCard ${isMrWhite ? "whiteRole" : ""}`}>
...
function ClueTurn({ state, busy, perform, remaining }: { state: PublicState; busy: boolean; perform: (a: string, x?: Record<string, unknown>) => Promise<void>; remaining: number | null }) {
  ...
  const isMe = g.currentTurnId === state.me.id && !state.me.isSpectator;
  const current = playerName(state, g.currentTurnId);
  const isTie = g.phase === "tiebreak-clue";
  ...
  return (
    <form className="yourTurn" onSubmit={submit}>
...
function Vote({ state, busy, perform }: { state: PublicState; busy: boolean; perform: (a: string, x?: Record<string, unknown>) => Promise<void> }) {
  const g = state.game!;
  const hasVoted = g.submittedVoterIds.includes(state.me.id);
  const eligible = g.eligibleVoterIds.includes(state.me.id) && !state.me.isSpectator;
  const targetPlayers = state.players.filter((p) => g.allowedTargetIds.includes(p.id));
  const isTie = g.phase === "tiebreak-vote";
  ...
  return (
    <div className="voteBox">
      <p>{isTie ? "Choisis qui éliminer parmi les joueurs encore à égalité." : "Choisis la personne que tu veux éliminer. Tu peux voter pour toi-même."}</p>
      <div className="voteTargets">
        {targetPlayers.map((p) => <button key={p.id} className="voteTarget" disabled={busy} onClick={() => perform("vote", { targetId: p.id })}><span className="avatar small">{p.name.slice(0, 1).toUpperCase()}</span><strong>{p.name}{p.id === state.me.id ? " (toi)" : ""}</strong><span>Voter →</span></button>)}
      </div>
      <VoteProgress state={state} />
    </div>
  );
}

function VoteProgress({ state }: { state: PublicState }) {
  const g = state.game!;
  const connectedEligible = g.eligibleVoterIds.filter((id) => state.players.find((p) => p.id === id)?.connected);
  const submitted = connectedEligible.filter((id) => g.submittedVoterIds.includes(id)).length;
  return <div className="voteProgress"><div><span style={{ width: `${connectedEligible.length ? (submitted / connectedEligible.length) * 100 : 0}%` }} /></div><small>{submitted}/{connectedEligible.length} votes reçus</small></div>;
}

function MrWhiteGuess({ state, busy, perform }: { state: PublicState; busy: boolean; perform: (a: string, x?: Record<string, unknown>) => Promise<void> }) {
  const g = state.game!;
  const [guess, setGuess] = useState("");
  const isMe = g.pendingMrWhiteId === state.me.id;
  const name = playerName(state, g.pendingMrWhiteId);
  if (!isMe) return <div className="waitingCard"><div className="pulseIcon">?</div><strong>{name} tente de deviner le mot</strong><p>S'il trouve exactement le mot civil, les Mr White gagnent.</p></div>;
  return (
    <form className="guessCard" onSubmit={(e) => { e.preventDefault(); if (guess.trim()) perform("mrwhiteGuess", { guess }); }}>
      <div className="roleTag">MR WHITE</div><h3>Une seule tentative.</h3><p>Écris le mot exact des civils. Les majuscules/minuscules ne comptent pas.</p>
      <div className="clueInputRow"><input autoFocus className="textInput clueInput" maxLength={60} value={guess} onChange={(e) => setGuess(e.target.value)} placeholder="Mot des civils…" /><button className="primaryButton" disabled={busy || !guess.trim()}>Valider</button></div>
    </form>
  );
}

function GameOver({ state, busy, perform }: { state: PublicState; busy: boolean; perform: (a: string, x?: Record<string, unknown>) => Promise<void> }) {
  const g = state.game!;
  const infiltrators = state.players.filter((p) => p.revealedRole === "infiltrator");
  return (
    <div className={`gameOverCard ${g.winner === "infiltrators" ? "infilWin" : ""}`}>
      <div className="winnerIcon">{g.winner === "civilians" ? "✓" : "◈"}</div>
      <h3>{g.winner === "civilians" ? "Les civils gagnent" : `${state.settings.mode === "mrwhite" ? "Mr White" : "Les imposteurs"} gagnent`}</h3>
      <p>{g.winReason}</p>
      <div className="wordReveal"><div><small>MOT CIVIL</small><strong>{g.civilianWord}</strong></div>{state.settings.mode === "impostor" && <div><small>MOT IMPOSTEUR</small><strong>{g.infiltratorWord}</strong></div>}</div>
      <div className="infilReveal"><small>{state.settings.mode === "mrwhite" ? "MR WHITE" : "IMPOSTEUR(S)"}</small><strong>{infiltrators.map((p) => p.name).join(", ")}</strong></div>
      {state.me.isHost ? <button className="primaryButton" disabled={busy} onClick={() => perform("reset")}>Rejouer avec la room</button> : <div className="waitingText">L'hôte peut relancer une partie.</div>}
    </div>
  );
}

function VoteRevealCard({ state }: { state: PublicState }) {
  const r = state.game!.lastVoteReveal!;
  if (r.votes.length === 0) return <div className="voteReveal"><strong>Dernier vote : aucun vote reçu.</strong></div>;
  return (
    <details className="voteReveal" open={state.game!.phase === "tiebreak-clue" || state.game!.phase === "tiebreak-vote"}>
      <summary>Résultat du dernier vote <span>{r.tiedIds.length ? "Égalité" : r.eliminatedId ? `${playerName(state, r.eliminatedId)} éliminé` : ""}</span></summary>
      <div className="voteLines">{r.votes.map((v, i) => <div key={`${v.voterId}-${i}`}><strong>{playerName(state, v.voterId)}</strong><span>→</span><strong>{playerName(state, v.targetId)}</strong></div>)}</div>
    </details>
  );
}

function Clues({ state }: { state: PublicState }) {
  const g = state.game!;
  const groups = useMemo(() => {
    const map = new Map<string, typeof g.clues>();
    for (const clue of g.clues) {
      const key = clue.kind === "tiebreak" ? `Départage ${clue.cycle}` : `Cycle ${clue.cycle} · Manche ${clue.round}`;
      const arr = map.get(key) ?? []; arr.push(clue); map.set(key, arr);
    }
    return [...map.entries()];
  }, [g.clues]);
  return (
    <section className="clueHistory">
      <div className="historyTitle">Indices visibles</div>
      {groups.map(([label, clues]) => <div className="clueGroup" key={label}><small>{label}</small><div className="clueChips">{clues.map((c) => <div className="clueChip" key={c.id}><span>{playerName(state, c.playerId)}</span><strong>{c.text}</strong></div>)}</div></div>)}
    </section>
  );
}

function PlayersInGame({ state }: { state: PublicState }) {
  return (
    <section className="playersPanel">
      <div className="panelTitle"><span>Joueurs</span><small>{state.players.filter(p => p.connected).length} EN LIGNE</small></div>
      <div className="sidePlayers">{[...state.players].sort((a,b)=>a.joinOrder-b.joinOrder).map((p) => <div className={`sidePlayer ${!p.connected ? "offline" : ""}`} key={p.id}><span className="avatar tiny">{p.name.slice(0,1).toUpperCase()}</span><div><strong>{p.name}{p.id === state.me.id ? " (toi)" : ""}</strong><small>{p.id === state.hostId ? "Hôte" : p.isEliminated ? roleName(state, p.revealedRole) ?? "Éliminé" : p.isSpectator ? "Spectateur" : p.connected ? "En jeu" : "Absent"}</small></div>{p.isEliminated && <span className="skull">×</span>}</div>)}</div>
    </section>
  );
}

function Activity({ state }: { state: PublicState }) {
  const events = [...state.events].reverse().slice(0, 12);
  return (
    <section className="activityPanel">
      <div className="panelTitle"><span>Activité</span></div>
      <div className="activityList">{events.length === 0 ? <p className="muted">Rien pour l'instant.</p> : events.map((e) => <div className={`activityItem ${e.kind}`} key={e.id}><span /><p>{e.text}</p></div>)}</div>
    </section>
  );
}
