...
import { ensureFairMrWhiteStart } from "@/lib/mrwhite-guard";
...
function applySettings(room: Room, raw: Partial<Settings>) {
  ...
  if (raw.wordSource === "custom") { s.wordSource = "custom"; s.mode = "impostor"; }
  else if (raw.wordSource === "random") s.wordSource = "random";
  if (raw.mode === "mrwhite") {
    if (s.wordSource === "custom") throw new Error("Mr White n'est pas disponible avec des mots personnalisés.");
    s.mode = "mrwhite";
  } else if (raw.mode === "impostor") s.mode = "impostor";
  if (s.mode === "impostor" && s.wordSource === "custom") { s.impostorKnowsRole = false; s.infiltratorsKnowEachOther = false; }
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (action === "publicRooms") return NextResponse.json({ ok: true, rooms: await listPublicRooms() });
    if (action === "create") {
      const name = sanitizeName(body.name);
      if (name.length < 2) throw new Error("Choisis un pseudo d'au moins 2 caractères.");
      const access = normalizeAccess(body.access), player = newPlayer(name, 1, false);
      let room: Room | null = null;
      for (let attempt = 0; attempt < 12; attempt++) {
        const code = makeCode();
        const candidate: Room = { code, createdAt: Date.now(), updatedAt: Date.now(), status: "lobby", hostId: player.id, nextJoinOrder: 2, access, joinRequests: [], settings: { ...DEFAULT_SETTINGS }, players: [player], game: null, events: [] };
    ...
    if (action === "join") {
      ...
      return NextResponse.json(await withRoomLock(code, (room) => {
        updatePresenceAndTimers(room); ensureFairMrWhiteStart(room);
        ...
        room.joinRequests.push(request); addEvent(room, `${name} demande à rejoindre la room privée.`, "join");
    ...
    if (action === "joinStatus") {
      ...
      return NextResponse.json(await withRoomLock(code, (room) => {
        ...
        updatePresenceAndTimers(room, player.id); ensureFairMrWhiteStart(room);
    ...
    return NextResponse.json(await withRoomLock(code, (room) => {
      ...
      room.access ??= "code"; room.joinRequests ??= []; pruneJoinRequests(room);
      if (action === "leaveSignal") { player.leaveSignaledAt = Date.now(); player.connected = false; return { ok: true }; }
      updatePresenceAndTimers(room, player.id);
      if (action === "state") { ensureFairMrWhiteStart(room); return { ok: true, state: publicState(room, player) }; }
      if (action === "ready") {
        if (room.status !== "lobby" || player.isSpectator) throw new Error("Impossible de changer ton statut maintenant."); player.ready = Boolean(body.ready);
        ...
        applySettings(room, body.settings ?? {}); resetReady(room); addEvent(room, "Les réglages ont changé : tout le monde repasse en Pas prêt.", "info");
      } else if (action === "transferHost") {
        if (room.hostId !== player.id) throw new Error("Seul l'hôte peut transférer le rôle d'hôte.");
        const targetId = String(body.targetId ?? ""), target = room.players.find((candidate) => candidate.id === targetId);
        ...
        room.hostId = target.id; addEvent(room, `${player.name} a transféré le rôle d'hôte à ${target.name}.`, "info");
        ...
        const joined = addDirectPlayer(room, request.name); request.status = "accepted"; request.playerId = joined.id; request.playerToken = joined.token; addEvent(room, `${request.name} a été accepté dans la room.`, "join");
      } else if (action === "rejectJoinRequest") {
        if (room.hostId !== player.id) throw new Error("Seul l'hôte peut refuser les demandes.");
        const request = room.joinRequests.find((item) => item.id === String(body.requestId ?? ""));
        if (!request || request.status !== "pending") throw new Error("Demande introuvable ou déjà traitée.");
        request.status = "rejected"; addEvent(room, `La demande de ${request.name} a été refusée.`, "info");
      } else if (action === "start") {
        if (room.hostId !== player.id) throw new Error("Seul l'hôte peut lancer la partie."); startGame(room);
      } else if (action === "ackReveal") ackReveal(room, player);
      else if (action === "clue") submitClue(room, player, String(body.clue ?? ""));
      else if (action === "vote") submitVote(room, player, String(body.targetId ?? ""));
      else if (action === "mrwhiteGuess") submitMrWhiteGuess(room, player, String(body.guess ?? ""));
      else if (action === "reset") {
        if (room.hostId !== player.id) throw new Error("Seul l'hôte peut relancer une partie.");
        if (room.status !== "gameover") throw new Error("La partie n'est pas terminée."); resetToLobby(room);
